...
class ChatbotService:
    ...
    def _handle_interrogation(self, user_message: str, suspect_id: str) -> dict:
        try:
            ...
            # 현재 질문까지 포함하여 대화 기록을 구성
            history = self._get_conversation_history(suspect_id, user_message)
            final_prompt = ""

            # 프롬프트 구조를 간결하고 명확하게 수정
            if retrieved_doc:
                fact_to_use = retrieved_doc['fact']
                lie_behavior = retrieved_doc['lie_behavior']
                final_prompt = f"""
# Master Instruction
당신은 용의자 '{suspect_config['name']}'입니다. 탐정 'Adrian Vale'과 대화하고 있습니다. 당신의 역할에 완벽하게 몰입하여, 자연스럽게 대답하세요. 절대로 AI이거나 연기 중이라는 사실을 드러내지 마세요.
...
# Conversation
{history}
{suspect_config['name']}: """ # LLM이 답변을 바로 생성하도록 유도
            else:
                final_prompt = f"""
# Master Instruction
...
{suspect_config['name']}: """
            ...
            print(f"[Fatal Error] LLM 응답 생성 중 오류 발생: {e}")
            import traceback
            traceback.print_exc()
            return {"reply": "죄송합니다. 생각에 잠시 오류가 생긴 것 같습니다. 다시 질문해주시겠어요?", "sender": suspect_id}
    ...
    def _search_similar(self, query: str, knowledge_base: list) -> dict | None:
        """
        사용자의 질문(query)에서 단어를 추출하여, knowledge_base의 keywords와 비교합니다.
        더 유연한 검색을 위해 양방향으로 확인합니다.
        """
        ...
        # 1점 이상일 때만 (즉, 하나라도 단어가 겹칠 때만) 유효한 검색으로 인정
        if max_score > 0:
            return best_match
    ...
    def _get_conversation_history(self, suspect_id: str, current_user_message: str, limit: int = 4) -> str:
        ...
        # 현재 사용자의 질문을 마지막에 추가하여 LLM이 답변할 차례임을 명확히 함
